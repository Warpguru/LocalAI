...
def main():
    ...
    for field in reader.fields.values():
        if "chat_template" in field.name:
            # The field's value is the last item in the 'parts' list
            value = field.parts[-1]
            if hasattr(value, 'tobytes'):  # It's a numpy array
                # Trim trailing null bytes before decoding
                chat_template = value.tobytes().rstrip(b'\x00').decode('utf-8', errors='ignore')
            elif isinstance(value, bytes):
                chat_template = value.decode("utf-8", errors="ignore")
            else:
